...
const upload = multer({
  storage: multer.diskStorage({ // 서버의 디스크에 저장
    destination(req, file, cb) {
      cb(null, 'uploads/'); //저장되는 경로
    },
    filename(req, file, cb) {
       // 파일명을 정해줘야함 안그럼 임의의 문자가 들어감
       // extname : 확장자, basename:파일명 
       // 파일명 중복을 피하기 위해 파일명에 현재시간을 추가한다.
      const ext = path.extname(file.originalname);
      cb(null, path.basename(file.originalname, ext) + new Date().valueOf() + ext);
    },
  }),
  limits: { fileSize: 5 * 1024 * 1024 }, //용량 제한 10 Mb
});
